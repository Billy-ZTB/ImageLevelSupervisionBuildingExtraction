...
class Backbone(nn.Module, ABC_Model):
    def __init__(self, model_name, num_classes=2, mode='fix', segmentation=False):
        super().__init__()
        ...
        if 'resnet' in model_name:
            ...
            # state_dict = torch.load(r'D:\WSSS_code\PuzzleCAM-master\experiments_official\models\ResNet50@Puzzle@optimal.pth')
            ...
            self.model.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=2, padding=1, bias=False) ##resnest50是64，其他是128
...
class Classifier(Backbone):
    def __init__(self, model_name = 'resnet50', num_classes = 2, mode='fix'):
        super().__init__(model_name, num_classes, mode)
        print('\n **************************************\nUsing Simplest Classifier\n**************************************')
        self.classifier = nn.Conv2d(2048, num_classes, 1, bias=False)
        self.num_classes = num_classes
        self.initialize([self.classifier])


    def forward(self, x, with_cam=True, separate = False):
        N, C, H, W = x.size()
        ...
        ######################
        # x_look = x.cpu().numpy()
        # x_look = x_look[1,:,:,:].reshape((2048,16,16))
        # x_look = x_look * 255 / x_look.max()
        # for i in range(2048):
        #     cv2.imwrite(r'D:\Experiments\look\baseCAM_painting\{}.png'.format(i),x_look[i,:,:])
...
class Classifier_For_Positive_Pooling(Backbone):
    def __init__(self, model_name, num_classes=2, mode='fix'):
        super().__init__(model_name, num_classes, mode)
...
class DeepLabv3_Plus(Backbone):
    def __init__(self, model_name, num_classes=2, mode='fix', use_group_norm=False):
